...
from podcatch_app.postgres_dump import (connect_postgres, dump_postgres_memory, save_postgres)
...
def add_podcast(cid=-1, cname='', furl='', port=5432):
    ...
    pod = Podcasts(
        castid=24,
        castname=u'Welcome to Night Vale',
...
def parse_feed(feed_it, cur_urls, newepid, pod_):
    ...
    if _pep.epurl and ((_pep.epfname() not in cur_urls) or
                       (_pep.epfname() in cur_urls and
                        cur_urls[_pep.epfname()].status not in ('Downloaded', 'Skipped'))):
        yield _pep
